...
router.post('/addevent', async (req,res) => {
 ...
 router.post('/:id/addfeedback', async (req,res) => {
    try {
     ...
     if (isValidated.error) return res.status(400).send({ error: isValidated.error.details[0].message })   
     Event.update({"_id": id},{$push:{"feedback": [req.body]}}).exec()
     res.json({msg:'Feedback was created successfully'})
    }
    catch(error) {
        // We will be handling the error later
        console.log(error)
 ...
 router.post('/:id/addphoto', async (req,res) => {
   ...
   if (isValidated.error) return res.status(400).send({ error: isValidated.error.details[0].message })
     Event.update({"_id": id},{$push:{"photos": [{"link":req.body.photos.link}]}}).exec()
