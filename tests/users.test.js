...
describe("Registration", async () => {
    ...
    test("duplicate email", () => {
        ...
        return request(app)
            ...
            .send(newUser)
            .then(response => {
                expect(response.body.error).toEqual("An account with this email already exists");
            });
    ...
    test("duplicate GUC ID", () => {
        ...
        return request(app)
            ...
            .send(newUser)
            .then(response => {
                expect(response.body.error).toEqual("An account with this guc id already exists");
            })
    ...
    describe("own profile", async () => {
        ...
        test("duplicate email", () => {
            return request(app)
                ...
                .set("Authorization", `${token}`)
                .then(res => {
                    expect(res.body.error).toEqual("An account with the requested email already exists")
                })
        })
        describe("successes", async () => {
            afterAll(async (done) => {
                try {
                    await User.findOneAndUpdate({ guc_id: "00-0001" }, { first_name: "test" }, { upsert: false });
                } catch (err) {
                    console.log(err)
                }
                done()
        ...
        describe("edit misc", async () => {
            describe("give admin", async () => {
                ...
                describe("successes", async () => {
                    afterAll(async (done) => {
                        try {
                            await User.findOneAndUpdate({ guc_id: "00-0001" }, { is_admin: false }, { upsert: false });
                        } catch (err) {
                            console.log(err)
                        }
                        done()
            ...
            describe("give awg admin", async () => {
                test("this user not an awg admin", () => {
                    return request(app)
                        ...
                        .set("Authorization", `${token2}`)
                        .then(res => {
                            expect(res.body.error).toEqual("this user is not an admin of any AWG")
                        })
                })
                test("user not found", () => {
                    return request(app)
                ...
                describe("successes", async () => {
                    afterAll(async (done) => {
                        try {
                            await User.findOneAndUpdate({ guc_id: "00-0001" }, { awg_admin: "none" }, { upsert: false });
                        } catch (err) {
                            console.log(err)
                        }
                        done()
...
describe("User Search", () => {
    ...
    test("Search for private user", () => {
        return request(app)
            ...
            .send({ searchkey: "two" })
            .then(res => {
                var flag = res.body.message == "No relevant data. Try searching with another keyword"
